...
export function creditTransactionDetailItems(record: CreditDisplayTransaction) {
    ...
    return [
        ...
        { label: "任务", value: detail.task_id },
        { label: "渠道", value: detail.channel_id ? `#${detail.channel_id}` : "" },
        { label: "渠道模型", value: detail.channel_model_id ? `#${detail.channel_model_id}` : "" },
        { label: "原扣费流水", value: detail.refund_of_transaction_id ? `#${detail.refund_of_transaction_id}` : "" },
        { label: "失败原因", value: detail.error },
...
export function creditTransactionSummary(record: CreditDisplayTransaction) {
    const detail = parseCreditMetadata(record.metadata);
    const items = [record.note, detail.total_cost ? `合计 ${detail.total_cost} 积分` : "", detail.formula ? detail.formula : ""].filter(Boolean);
