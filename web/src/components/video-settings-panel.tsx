...
export function videoSettingsOverview(config: AiConfig, selectedModel?: string, runtime?: CustomVideoRuntimeSnapshot) {
    ...
    if (videoRouteForModel(resolvedConfig, model) === "custom") {
        ...
        return values.join(" · ") || "自定义视频";
    }
    const displaySeconds = normalizeVideoDurationForModel(config, model, config.videoSeconds);
    return `${videoResolutionLabel(config.vquality)} · ${videoSizeLabel(config.size)} · ${videoSecondsLabel(displaySeconds)}`;
}
        ...
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title="秒数" color={theme.node.muted}>
                    ...
                    {!fixedDuration && configuredDurations.length > 0 ? (
                        <div className="text-[11px] leading-4 opacity-55">
                            当前模型可选时长：{configuredDurations.join(" / ")} 秒{customizable ? "，也可自定义。" : "。"}{" "}
                        </div>
                    ) : null}
                </SettingGroup>
            </div>
...
function NumberInput({ value, min, max, disabled = false, theme, onChange }: { value: string; min: number; max: number; disabled?: boolean; theme: CanvasTheme; onChange: (value: string) => void }) {
    return (
        <input
            ...
            max={max}
            disabled={disabled}
            className="h-9 rounded-full border bg-transparent px-3 text-center text-sm outline-none disabled:cursor-not-allowed disabled:opacity-60 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
