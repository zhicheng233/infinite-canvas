...
describe("video settings overview", () => {
    test("uses the selected custom size and runtime duration", () => {
        ...
        expect(videoSettingsOverview(config, customModel, { values: { dimension: "720x1280", seconds: 10 }, media: {} })).toBe("720x1280 · 10s");
    });
    ...
    test("uses the selected custom aspect ratio", () => {
        ...
        expect(videoSettingsOverview(config, customModel, { values: { dimension: "9:16" }, media: {} })).toBe("9:16 · 6s");
    });
    ...
    test("switches between standard and custom model values without stale custom state", () => {
        ...
        expect(videoSettingsOverview(standardConfig, "standard-video", { values: { dimension: "720x1280", seconds: 10 }, media: {} })).toBe("1080p · 方形 · 12s");
        expect(videoSettingsOverview(standardConfig, customModel, { values: { dimension: "720x1280", seconds: 10 }, media: {} })).toBe("720x1280 · 10s");
    });

    test("normalizes stale runtime values against the current custom model", () => {
        const config = configWithCustomVideo(customConfig());

        expect(videoSettingsOverview(config, customModel, { values: { dimension: "9:16", seconds: 20 }, media: {} })).toBe("1280x720 · 6s");
    });
