...
export default function VideoPage() {
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        const nextAudioReferences = videoRouteForModel(effectiveConfig, model) === "seedance" ? filterAudioReferencesByDuration(audioReferences, uploadedAudioReferences, message.warning) : uploadedAudioReferences;
    ...
    const deleteSelectedLogs = () => {
        ...
        void Promise.all([deleteStoredMedia(mediaKeys), deleteGenerationRecords("video", selectedLogIds)]).then(refreshLogs);
    ...
    const saveLog = async (log: GenerationLog) => {
        await saveGenerationRecord("video", serializeLog(log));
        await refreshLogs();
    };
                        ...
                        <div className="mt-6 space-y-5">
                                            ...
                                            {references.map((item, index) => (
                                                <div key={item.id} className="group relative size-20 shrink-0 overflow-hidden rounded-md border border-stone-200 dark:border-stone-800">
                                                    ...
                                                    <span className="absolute left-1 top-1 rounded bg-black/60 px-1.5 py-0.5 text-[10px] font-medium text-white">
                                                        {imageReferenceMode === "first_last" && index < 2 ? (index === 0 ? "首帧" : "尾帧") : seedanceReferenceLabel("image", index)}
                                                    </span>
                            ...
                            <div className="hidden gap-4 sm:grid sm:grid-cols-2">
                                <GenerationSettings
                                    ...
                                    customVideoRuntime={normalizedCustomVideoRuntime}
                                    onCustomVideoRuntimeChange={setCustomVideoRuntime}
                                    onCustomVideoMediaRoleOpen={(role) => document.getElementById(`custom-video-role-${role}`)?.scrollIntoView({ behavior: "smooth", block: "center" })}
                                />
                            </div>
                        </div>
        ...
        <>
            ...
            <div className="col-span-2">
                <VideoSettingsPanel
                    ...
                    onCustomVideoMediaRoleOpen={onCustomVideoMediaRoleOpen}
                />
            </div>
        </>
    );
...
async function readStoredLogs() {
    try {
        const logs = await listGenerationRecords<GenerationLog>("video");
...
function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        videoReferenceMode: normalizeBinghuoReferenceMode(log.config?.videoReferenceMode),
    };
}
...
function buildLog({
    ...
    const logConfig = {
        ...
        videoReferenceMode: normalizeBinghuoReferenceMode(config.videoReferenceMode),
    };
    return {
        id: nanoid(),
        createdAt: Date.now(),
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    const route = videoRouteForModel(config, model);
    const binghuo = route === "binghuo";
    const seedance = !binghuo && (route === "seedance" || isSeedanceVideoConfig({ ...config, model }));
    return {
        ...config,
        model,
        videoModel: model,
        size: binghuo ? normalizeBinghuoRatio(config.size) : seedance ? normalizeSeedanceRatio(config.size) : normalizeVideoSize(config.size),
        videoSeconds: normalizeVideoSeconds(config.videoSeconds),
        vquality: binghuo ? normalizeBinghuoResolution(config.vquality) : normalizeResolution(config.vquality),
