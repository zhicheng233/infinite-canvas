...
import { Users, DollarSign, CreditCard, ReceiptText, ArrowLeft, Shield, LayoutDashboard } from "lucide-react";
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <Layout className="h-full bg-background" hasSider>
      <Sider
        width={200}
        className="border-r border-stone-200 dark:border-stone-800 bg-background"
        theme="light"
      >
        <div className="flex flex-col h-full">
          ...
          <div className="border-t border-stone-200 dark:border-stone-800 p-3">
            <button
              onClick={() => router.push("/")}
              className="flex items-center gap-2 text-xs text-stone-500 hover:text-stone-800 dark:hover:text-stone-200 w-full px-3 py-2 rounded transition-colors"
            >
