...
export default function AdminModelLogsPage() {
  ...
  const fetchLogs = useCallback(async (page = pagination.current, pageSize = pagination.pageSize) => {
    setLoading(true);
    try {
  ...
  const columns: ColumnsType<ModelCallLogItem> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="min-w-0">
          ...
          <div className="text-xs text-stone-500">ID: {record.user_id}{record.username ? ` · ${record.username}` : ""}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      width: 260,
      ellipsis: true,
      render: (_, record) => <span className="font-mono text-xs">{record.method} {record.path}</span>,
    },
    {
      title: "状态",
  ...
  return (
    <div>
      ...
      <Card className="mb-5" title="模型健康概览" extra={<Button size="small" icon={<RotateCcw className="size-4" />} onClick={() => void fetchHealth()}>刷新</Button>} loading={healthLoading}>
      ...
      <Form form={form} layout="inline" className="mb-4 gap-y-3" initialValues={{ generation: "" }} onFinish={() => fetchLogs(1, pagination.pageSize)}>
        ...
        <Form.Item>
          <Space>
            <Button type="primary" htmlType="submit" icon={<Search className="size-4" />}>查询</Button>
            <Button
        ...
        {selected ? (
          <div className="space-y-3">
            <div className="grid gap-2 text-sm md:grid-cols-2">
              <div><Text type="secondary">用户：</Text>{selected.display_name || selected.username || `#${selected.user_id}`}</div>
              <div><Text type="secondary">模型：</Text>{selected.model || "-"}</div>
              <div><Text type="secondary">类型：</Text>{generationLabels[selected.generation] || selected.generation || "-"}</div>
              <div><Text type="secondary">状态：</Text>{selected.status_code || "本地错误"}</div>
              <div className="md:col-span-2"><Text type="secondary">接口：</Text><span className="font-mono">{selected.method} {selected.path}</span></div>
              <div className="md:col-span-2"><Text type="secondary">错误：</Text>{selected.error_message || "-"}</div>
            </div>
