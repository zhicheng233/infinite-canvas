        ...
        <div className="flex h-full w-full cursor-move flex-col px-3 pb-3 pt-7 text-sm" style={{ color: theme.node.text }} onWheel={(event) => event.stopPropagation()}>
                ...
                <div className="cursor-default" onMouseDown={(event) => event.stopPropagation()}>
                    <Segmented
                        ...
                        onChange={(value) => {
                            ...
                            const model = resolveCapabilityModel(globalConfig, nextMode, node.metadata?.model);
                            onConfigChange(node.id, {
                                generationMode: nextMode,
                                model,
                                customVideoRuntime: nextMode === "video" ? canvasCustomVideoRuntimeForModel(globalConfig, model, node.metadata?.customVideoRuntime) : undefined,
                            });
                        }}
                        options={[
                            {
            ...
            <div className={`mb-2 grid min-w-0 cursor-default items-center gap-2 ${mode === "image" || mode === "video" || mode === "audio" ? "grid-cols-[minmax(0,1fr)_148px]" : "grid-cols-1"}`} onMouseDown={(event) => event.stopPropagation()}>
                <ModelPicker
                    ...
                    config={config}
                    value={config.model}
                    onChange={(model) => onConfigChange(node.id, { model, customVideoRuntime: mode === "video" ? canvasCustomVideoRuntimeForModel(config, model, node.metadata?.customVideoRuntime) : undefined })}
                    capability={mode}
