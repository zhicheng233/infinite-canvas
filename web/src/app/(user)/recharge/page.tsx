...
export default function RechargePage() {
    ...
    const transactionColumns = [
        ...
        },
        {
            ...
            key: "detail",
            width: 340,
            render: (_: unknown, record: { id?: number; type?: string; amount?: number; balance_before?: number; balance_after?: number; ref_type?: string; ref_id?: string; note?: string; metadata?: string; created_at?: string }) => (
                <CreditTransactionDetailButton record={record} />
            ),
        },
        {
            title: "时间",
        ...
        <main className="mx-auto max-w-5xl overflow-y-auto px-6 py-8">
            ...
            <div className="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                {payouts.map((payout) => {
                    ...
                    return (
                        <div key={payout.id} className={cn("relative flex flex-col rounded-2xl border bg-gradient-to-b p-5 transition hover:shadow-lg", colors.bg, colors.border)}>
            ...
            <Card>
                <Table dataSource={orders} columns={orderColumns} rowKey="id" loading={ordersLoading} pagination={false} locale={{ emptyText: "暂无充值记录" }} />
            </Card>
            ...
            <Card>
                <Table dataSource={transactions} columns={transactionColumns} rowKey="id" loading={transactionsLoading} pagination={false} locale={{ emptyText: "暂无积分流水" }} scroll={{ x: 1080 }} />
            </Card>
        </main>
    );
}
