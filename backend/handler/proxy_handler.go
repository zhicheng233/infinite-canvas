...
func (h *ProxyHandler) Proxy(c *gin.Context) {
	...
	result, err := h.generateService.ProxyRaw(claims.TenantID, claims.UserID, method, targetPath, contentType, body)
	if err != nil {
...
func (h *ProxyHandler) ProxyGet(c *gin.Context) {
	...
	result, err := h.generateService.ProxyRaw(claims.TenantID, claims.UserID, "GET", targetPath, "", nil)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"code": 500, "msg": err.Error()})
		return
	}
