...
本文档说明当前画布的本地缓存、账号云端同步、文档版本、图片文件存储和兼容边界。
...
- `id`：画布项目 ID，当前前端生成。
- `schemaVersion`：画布文档结构版本；无版本文档定义为版本 1，并按顺序经过纯迁移函数升级。
- `revision`：云端乐观锁修订号；本地新画布为 0。
- `title`：画布名称。
...
type CanvasConnection = {
  ...
  fromNodeId: string;
  toNodeId: string;
  targetImageRole?: "images" | "input_reference" | "style_references" | "element_references" | "reference_images";
};
```

连线只保存节点 ID 和可选图片角色，不保存端口坐标。缺省角色是兼容图片槽；`targetImageRole: "images"` 是读取兼容别名，保存时会规范化为缺省角色。其他显式角色在当前模型不支持时仍保留，切回支持模型后恢复。

删除节点时会同步删除以该节点为起点或终点的连线。删除图片组根节点时，会把对应子节点一起删除。

## 文档版本、导出与云端冲突

- 当前画布文档版本是 2；版本 1 到版本 2 会规范化图片角色连线并去重。
- 画布 ZIP 导出 envelope 保持 `version: 3`，其中每个项目仍携带自己的 `schemaVersion`。
- 云端保存携带预期 `revision`。服务端版本不一致时返回 409 和最新云端项目，前端保留当前内容为“本地冲突副本”，不会静默覆盖。
- 本地保存定时器和云端保存请求按“账号作用域 + 项目 ID”隔离；账号切换时取消旧作用域的定时器和请求。
